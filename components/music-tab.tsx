...
import { MoreHorizontal, Music4, Plus, Pencil, Trash2, X } from "lucide-react";
...
export default function MusicTab() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    let firstAddedTrack: Track | null = null;
    ...
    for (const file of Array.from(files)) {
      ...
      const addedTrack = await addTrack(file);

      if (!firstAddedTrack && addedTrack) {
        firstAddedTrack = addedTrack;
      }
    }
    ...
    if (firstAddedTrack) {
      setRenameState({
        trackId: firstAddedTrack.id,
        value: firstAddedTrack.name,
      });
    }
  }

  function openRenameModal(track: Track) {
    setRenameState({
      trackId: track.id,
      value: track.name,
    });
  ...
  function submitRename() {
    ...
    renameTrack(renameState.trackId, renameState.value);
    setRenameState(null);
  }
    ...
    <>
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <p className="mt-1 text-xs theme-faint md:hidden">
                          MP3
                        </p>
                      </button>
      ...
      {renameState ? (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 px-4">
          <div className="theme-panel w-full max-w-md rounded-[28px] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
            <div className="mb-5 flex items-center justify-between gap-4">
              <div>
                <p className="text-sm theme-muted">Music library</p>
                <h3 className="text-2xl font-semibold">Rename track</h3>
              </div>

              <button
                type="button"
                onClick={() => setRenameState(null)}
                className="theme-button-soft grid h-10 w-10 place-items-center rounded-2xl"
            ...
            <label className="grid gap-2">
              <span className="text-sm theme-muted">Track name</span>
              <input
                autoFocus
            ...
            <div className="mt-5 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setRenameState(null)}
                className="theme-button-soft rounded-2xl px-4 py-3 text-sm font-medium"
              >
                Cancel
              </button>
